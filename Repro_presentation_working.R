...
(Ripe_0.33 <- Overall_counts %>% group_by(Year, Month, Final_Stage) %>%
    ...
    filter(meanProp3 > 0.333 & meanProp3 < 0.5)) 
#
(Ripe_First_Last0.5 <- Overall_counts %>% group_by(Year, Month, Final_Stage) %>%
...
#
#(Ripe_monthly <- full_join(Ripe_First_Last0.333, Ripe_First_Last0.5) %>% 
#    mutate(Year = as.factor(Year), 
#           Type = as.factor(ifelse(meanProp3 >0 & is.na(meanProp5), "33%", ifelse(meanProp3 >0 & meanProp5 >0, "50%", "Z")))) %>% 
#    ungroup() %>% complete(Year, Month))
#
...
Overall_counts %>% group_by(Year, Month, Final_Stage) %>%
  ...
  geom_rect(data = Ripe_First_Last0.5, aes(xmin = as.numeric(Month)-0.5, xmax = as.numeric(Month)+0.5, ymin = -Inf, ymax = Inf), 
